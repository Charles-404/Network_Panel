...
import { Shield, Cpu, MemoryStick, Thermometer, Activity, Clock, HardDrive } from 'lucide-react';

export function FortiGateCard({ status, className }: { status: FortiGateStatus; className?: string }) {
  const { system } = status;
  const stats = [
    { icon: Cpu, label: 'CPU', value: system.cpu + '%', color: system.cpu > 80 ? 'text-noc-red' : system.cpu > 60 ? 'text-noc-yellow' : 'text-noc-green' },
    { icon: MemoryStick, label: '内存', value: system.memory + '%', color: system.memory > 80 ? 'text-noc-red' : system.memory > 60 ? 'text-noc-yellow' : 'text-noc-green' },
    { icon: Thermometer, label: '温度', value: system.temperature + '°C', color: system.temperature > 60 ? 'text-noc-red' : system.temperature > 50 ? 'text-noc-yellow' : 'text-noc-green' },
